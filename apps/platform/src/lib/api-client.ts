...
function refreshAccessToken(): Promise<string> {
  const refreshToken = useSuperAdminSession.getState().refreshToken;
  ...
  if (!refreshInFlight) {
    refreshInFlight = fetch(buildUrl("/auth/refresh/"), {
      ...
      .then(async (res) => {
        ...
        useSuperAdminSession.getState().setTokens(tokens);
...
async function requestUrl<T>(url: string, options: RequestOptions = {}): Promise<T> {
  ...
  const accessToken = useSuperAdminSession.getState().accessToken;
  ...
  if (res.status === 401 && !_retried && useSuperAdminSession.getState().refreshToken) {
    try {
      await refreshAccessToken();
    } catch {
      useSuperAdminSession.getState().logout();
